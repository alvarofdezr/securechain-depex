...
class RepositoryInitializer:
    ...
    def __init__(self):
        ...
        self.requirement_file_service: RequirementFileService = (
            container.get_requirement_file_service()
        )
        self.package_service: PackageService = container.get_package_service()
        self.repo_analyzer: RepositoryAnalyzer = RepositoryAnalyzer(
            container.get_http_session()
        )
    ...
    async def init_repository(
        ...
        if repository is None:
            ...
            repository_id = await self.repository_service.create_repository(
                repository_data
            )

            await self.extract_repository(raw_requirement_files, repository_id)

            await self.repository_service.update_repository_is_complete(
                repository_id, True
            )
        else:
            repository_id = repository.get("id", "")

            needs_update = not repository["moment"] or repository["moment"].replace(
                tzinfo=UTC
    ...
    async def extract_repository(
        self, raw_requirement_files: dict, repository_id: str
    ) -> None:
    ...
    async def replace_repository(
        ...
        """
        existing_files = (
            await self.requirement_file_service.read_requirement_files_by_repository(
                repository_id
            )
        )
        ...
        for file_name, requirement_file_id in existing_files.items():
            if file_name not in raw_requirement_files:
                await self.requirement_file_service.delete_requirement_file(
                    repository_id, file_name
                )
            else:
    ...
    async def update_requirement_file(
        ...
        """
        existing_packages = (
            await self.package_service.read_packages_by_requirement_file(
                requirement_file_id
            )
        )
            ...
            if package_name in new_packages:
                if constraints != new_packages[package_name]:
                    await self.requirement_file_service.update_requirement_rel_constraints(
                        requirement_file_id, package_name, new_packages[package_name]
                    )
                new_packages.pop(package_name)
            else:
                await self.requirement_file_service.delete_requirement_file_rel(
        ...
        await self.requirement_file_service.update_requirement_file_moment(
            requirement_file_id
        )

    async def process_requirement_file(
        self, name: str, file_data: dict, repository_id: str
    ) -> None:
        ...
        req_file_id = await self.create_requirement_file(name, manager, repository_id)

        await self.queue_packages(packages, manager, req_file_id)

    async def create_requirement_file(
        self, name: str, manager: str, repository_id: str
    ) -> str:
        ...
        return await self.requirement_file_service.create_requirement_file(
            req_file_dict, repository_id
        )

    async def queue_packages(
        self, packages: dict[str, Any], manager: str, req_file_id: str
    ) -> None:
        ...
        for package_key, constraints in packages.items():
            ...
            if not await self.package_service.exists_package(node_type, package_name):
                ...
                packages_by_type.setdefault(node_type, []).append(
                    {"name": package_name, "constraints": constraints}
                )
